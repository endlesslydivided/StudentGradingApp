  ...
  async parseReportsCSV(file: Express.Multer.File): Promise<Array<Student>> {
    ...
    try {
      return this.sequelizeInstance.transaction(async (transaction) => {
        ...
        await this.gradesService.setLastSubmitted(transaction);
        ...
        studentsLines.forEach((line) => {
          ...
          for (let i = 0; i < reportSubjects.length; i++) {
            gradesData.push({
              mark: grades[i] === '' ? null : Number(grades[i]),
        ...
        return this.studentsService.upsertMany(studentReports, transaction);
      });
    } catch (error) {
      throw error;
    }
